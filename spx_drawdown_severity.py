...
``profit`` = ``Close``. For each window (30, 60, 90 days), histograms and
...
def analyze_spx_drawdown_percentiles(
    ...
    for ax, p in zip(axes_flat, periods):
        ...
        dd, rolling_max, _ = _compute_drawdown_series(
            profit_ref, profit_actual, period=p
        )
        ...
        pct = worst_case_tail_percentiles(ser)
        x = ser.dropna().to_numpy(dtype=float)
        x = x[np.isfinite(x)]
        moments = moment_summary(x)
        out[p] = {
            "fractional_drawdown_series": ser,
            "drawdown_series": ser,
            "dollar_drawdown_series": dd,
        ...
        x_pct = x * 100.0
        ax.hist(x_pct, bins=80, color="steelblue", edgecolor="white", alpha=0.88)
        ax.axvline(0, color="black", linewidth=0.8, linestyle="--", zorder=2)
        ymax = ax.get_ylim()[1]
        # Worst tail: 1st / 5th / 10th pct (more negative = worse)
        specs = (
        ...
        horizon = f"{p} day horizon" if p == 1 else f"{p} day horizons"
        ax.set_title(f"Drawdown% in {horizon} (n={len(x)}; SPX Close vs rolling peak)")
        ax.set_xlabel("Drawdown % of peak = 100 × (Close − peak) / peak")
        ax.set_ylabel("Count")
